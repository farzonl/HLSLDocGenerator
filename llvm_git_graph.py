...
def extract_all_opcodes(text):
    matches = re.findall(r'def \w+ : DXILOpMapping<\s*(\d+)', text)
    if matches:
       return matches
    return []
...
def llvm_graph_hlsl_intrinsics(data=None):
    ...
    dates = [date for date, _, _, _ in data]
    intrinsics = [ int_count for _, int_count, _, _ in data]
    percentages = [ percentage for _, _, percentage, _ in data]
    fig = go.Figure()
...
def llvm_git_hlsl_parser():
    ...
    data = [
    ['Date',  '# of Intrinsics', f"% of Intrinsics", "Intrinsics"]
    ]
    intrinsics = []
    num_intrinsics = 0
    for date, file_version in hlsl_intrinsic_files:
        intrinsics = extract_all_hlsl_intrinsics(file_version)
        num_intrinsics = len(intrinsics)
        if num_intrinsics > 0:
            row = [date.strftime('%Y-%m-%d'), num_intrinsics, round(100 * (num_intrinsics / len(all_hlsl_intrinsic)),2), ', '.join(intrinsics)]
            data.append(row)
...
def llvm_git_dxil_parser():
    ...
    data = [
    ['Date',  '# of DXILOps', f"% of DXILOps", "DXILOp ids", "DXILOp Names"]
    ]
    opcodes = []
    num_opcodes = 0
    for date, file_version in dxil_opcode_files:
        opcodes = extract_all_opcodes(file_version)
        num_opcodes = len(opcodes)
        if num_opcodes > 0:
            row = [date.strftime('%Y-%m-%d'), num_opcodes, round(100 * (num_opcodes / len(all_dxil_ops)),2), ', '.join(opcodes), ', '.join(getopNames(opcodes))]
            data.append(row)
...
def llvm_graph_dxil_intrinsics(data=None):
    ...
    dxilops = [ dxil_count for _, dxil_count, _, _, _ in data]
    percentages = [  int_per for _, _,  int_per, _, _ in data]
    fig = go.Figure()
...
def get_git_commits(repo_path: str, file_path : str):
    ...
    while current_date <= last_commit_date:
        closest_commit = min(commits, key=lambda x: abs(x.committed_datetime - current_date))
    
        # Get the version of the file in the closest commit
        version = closest_commit.tree[file_path].data_stream.read().decode('utf-8')
        file_versions.append((current_date, version))
    
        current_date += week_delta
    return file_versions

llvm_graph_hlsl_intrinsics()